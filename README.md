...
- **Accurate label colors** — reads your per-clip label assignments directly from the project file
- **Multi-sequence support** — tab between sequences when a project contains more than one
...
| **Premiere Pro** | Any version using the `.prproj` format |
...
4. Reads `asl.clip.label.name` (slot key) and `asl.clip.label.color` (BGR-encoded snapshot) for each clip
5. Builds a per-slot color map from project-panel clips (via `MasterClip` references) to get canonical label colors, falling back to Premiere's 16 default label colors
6. Renders the timeline with Core Graphics — no WebKit, no AppKit views beyond a single `NSView`
